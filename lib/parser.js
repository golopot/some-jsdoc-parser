...
function consumeRegExp(parser, regExp, message) {
  ...
  if (!match) {
    ...
    throw new ParseError(msg, parser.pos);
  }
...
function parseType(parser) {
  ...
  while (curlyBalance > 0) {
    if (isEOF(parser)) {
      throw new ParseError("'}' expected.", parser.pos);
    }
...
function createParser(source) {
  ...
  if (commentStartMatch === null) {
    throw new ParseError('Input source must start with "/**".', 0);
  }

  const commentEndMatch = /\*\/[ \t\n\r]*$/.exec(source);
  if (commentEndMatch === null) {
    throw new ParseError(
      'Input source must ends with "*/".',
      source.length - 1
    );
  }
...
/**
 * @param {string} source A comment string that starts with `/**` and ends with `/`.
